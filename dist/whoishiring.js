...
var MXStorable = function(c, d, e) {
  d = void 0 === d ? !1 : d;
  var f = c || {};
  e = void 0 === e || e ? Object.assign({id:(f.lsPrefix || "MXSTOR_ANON") + uuidv4(), created:Date.now()}, f, {deleted:f.deleted || cI(null)}) : f;
  Model.call(this, null, null, e, !1);
  !d && c.id || this.store();
};
...
var mxXHR = function(c, d) {
  ...
  Model.call(this, null, "mxXhr", {uri:cI(c), xhr:cI(null), okResult:cI(null, {observer:d.okHandler})});
...
mxXHR.prototype.send = function(c) {
  var d = this, e = function() {
    d.sent = Date.now();
    goog.net.XhrIo.send(d.uri, function(c) {
      c = c.target;
      d.xhr = c;
      d.recd = Date.now();
      d.verbose && clg("xhr responded after", Date.now() - d.sent);
      if (c.isSuccess()) {
        ...
        if (c.isActive()) {
          clg("xhr still active", d.uri);
        } else {
          if (c.isComplete()) {
            clg("xhr complete", d.uri, c.getLastError());
          } else {
            throw clg("send XHR uh-oh", d.uri), "getXHR error " + c.getLastError() + " on xhr " + d.uri;
          }
        }
      }
    });
...
function xhrJson(c, d) {
  d = void 0 === d ? {} : d;
  return new mxXHR(c, Object.assign({send:!0, delay:0, responseType:"json"}, d));
}
function testXHR() {
  clg("testXHR");
...
function toggleChar(c, d, e, f, g, h, k, l) {
  h = void 0 === h ? {} : h;
  k = void 0 === k ? {} : k;
  return span(Object.assign({style:"font-weight:bold; cursor:pointer; margin-left:9px; font-family:Arial; font-size:1em;" + (void 0 === l ? "" : l), onclick:function(c) {
...
function openShutCase(c, d, e, f, g) {
  ...
  var l = c + "-toggle";
  return div(div({class:"selector"}, span(d), toggleChar(l, "Show/hide " + d, e, "&#x25be", "&#x25b8"), f), div({class:cF(function(c) {
...
function openCase(c, d, e) {
  ...
  return div(span({style:"margin-left:24px;min-width:48px"}, d), f.map(function(c) {
    return c();
  }));
}
...
function viewOnHN(c, d) {
  d = void 0 === d ? {} : d;
  return a(merge({href:c, title:"View on the HN site"}, d), img({src:"dist/hn24.png"}));
}
...
function mobileCheck() {
  var c = !1, d = navigator.userAgent || navigator.vendor || window.opera;
...
function helpList(c, d) {
  ...
  }}, "X"), ul({style:"list-style:none; margin-left:0;padding 6px"}, c.map(function(c) {
    return li({class:"helpli"}, c);
  })));
}
...
UserNotes.storableProperties = function() {
  return "id hnId excluded stars hidden applied notes".split(" ").concat(MXStorable.storableProperties.call(this));
};
UserNotes.loadFromStorage = function() {
  return mkm(null, "JobList", {dict:cF(function(c) {
    c = {};
    for (var d = MXStorable.loadAllItems(UserNotes, UNOTE) || [], e = 0; e < d.length; ++e) {
      var f = d[e];
      c[f.hnId] = f;
    }
    return c;
...
function userAnnotations(c) {
  return div({style:"display:flex; flex-direction: column"}, div({class:"userAnnotations"}, jobStars(c), applied(c), noteToggle(c), excludeJob(c)), noteEditor(c));
}
function noteToggle(c) {
  var d = UNote.dict[c.hnId];
  return span({style:cF(function(c) {
    return "margin-left:18px; cursor:pointer;color:" + (d.notes && 0 < d.notes.length ? "#f00" : "#000");
  }), title:"Show/hide editor for your own notes", onclick:function(c) {
    return c.editing = !c.editing;
  }}, {name:"note-toggle", editing:cI(0 < (d.notes || "").length)}, "Notes");
}
  ...
  return div({style:hzFlexWrapBottom}, function(d) {
    ...
    for (var e = UNote.dict[c.hnId], f = 0; f < MAX_STARS; ++f) {
      d.push(span({content:"&#x2605", style:cF(function(c) {
        return "margin-top:0px;cursor:pointer;color:" + (e.stars >= c.md.starN ? "red;" : "gray;");
      }), onclick:function(c) {
        c.fmUp("job-listing");
...
function applied(c) {
  return div({style:"display:flex; flex-wrap: wrap; align-items:center"}, input({id:"applied?" + c.hnId, type:"checkbox", style:"margin-left:18px", checked:cF(function(d) {
    return UNote.dict[c.hnId].applied || !1;
  }), onclick:function(d) {
    d = UNote.dict[c.hnId];
    d.applied = !d.applied;
  }}, {name:"applied?"}), label({for:"applied?" + c.hnId, style:cF(function(d) {
    return "color:" + (UNote.dict[c.hnId].applied ? "red" : "black");
  })}, "Applied"));
}
;Hiring.filtering = {};
...
function mkJobSelectsEx(c, d, e, f) {
  f = void 0 === f ? {} : f;
...
function jobSpec(c) {
  for (var d = {hnId:c.id, body:[]}, e = 0; e < c.children.length; ++e) {
    jobSpecExtend(d, c.children[e], 0);
  }
  return d;
}
function charCount(c, d) {
...
function pickAMonth() {
  ...
  })}), span({style:"color: #fcfcfc; margin: 0 12px 0 12px", hidden:cF(function(c) {
    return !c.md.fmUp("searchMonth").value;
...
function jobListingLoader() {
  return div({style:"visibility:collapsed;"}, {name:"jobLoader", jobs:cF(function(c) {
    ...
    return c.every(function(c) {
      return null !== c;
    }) ? (clg("all jobs resolved!!!!", c.map(function(c) {
      return c.length;
...
function mkExtraJobLoader(c) {
  return iframe({src:"dist/extras/" + c + ".html", style:"display: none", onload:function(d) {
    return extraJobParse(d, c);
  }}, {name:c + "loader", jobs:cI(null)});
}
function extraJobParse(c, d) {
  c.jobs = null;
  if (c.dom.contentDocument) {
    hnBody = c.dom.contentDocument.getElementsByTagName("body")[0];
    var e = Array.prototype.slice.call(hnBody.querySelectorAll(".athing"));
    0 < e.length ? (e[0].id = d, d = jobSpec(e[0]), d.OK ? (e = d.hnId, UNote.dict[e] = new UserNotes({hnId:e}), UNote.dict[e] || (UNote.dict[e] = new UserNotes({hnId:e})), c.jobs = [d]) : clg("extra not OK")) : clg("no job!!!!!!!!!");
  } else {
    clg("no content!!!!!!!!!");
  }
}
...
function frameZap(c) {
  b = c.dom.contentDocument.getElementsByTagName("body")[0];
  b.innerHTML = "";
}
;Hiring.controlPanel = {};
...
function sortBar() {
  return div({style:{padding:"0 0 0 0", margin:"15px 0 0 24px", display:"flex"}}, span("Sort &nbsp"), ul({style:merge(hzFlexWrap, {padding:"0 0 0 0", margin:"0 0 0 0"})}, {name:"sortby", order:cI(jobSorts[0].defaultOrder), selection:cI(jobSorts[0]), sortSpec:cF(function(c) {
    ...
    return button({style:cF(function(c) {
      return "color:" + (c.md.selected ? "blue" : "#222");
    }), class:"sortOption", selected:cF(function(d) {
...
function jobListingControlBar() {
  return div({style:merge(hzFlexWrapCentered, {margin:"12px 0 0 0px", font_size:"1em", padding:"4px", border_style:"groove", border_color:"khaki", border_width:"2px", background:"PAPAYAWHIP", justify_content:"space-between", align_items:"center"})}, div({style:merge(hzFlexWrapCentered, {flex_wrap:"wrap"})}, span({style:"font-size:1em;margin-right:12px", content:cF(function(c) {
    ...
    return c.onOff = !c.onOff;
  }, title:"Show/hide items you have excluded"}, {name:"showExcluded", onOff:cI(!1), excludedCt:cF(function(c) {
...
function jobHeader(c) {
  return div({style:"cursor:pointer; display:flex", onclick:function(c) {
    c = c.fmDown("showDetails");
    c.onOff = !c.onOff;
...
function jobDetails(c) {
  return div({onclick:function(c) {
    ...
    "triplebytefull" === c.hnId ? window.open("https://triplebyte.com/a/R1E80IU/d", "_blank") : jumpToHN(c.hnId);
  }}, function(d) {
...
function mkListingRgx(c, d, e, f) {
  return labeledRow(d, input({placeholder:"Regex for " + e + " search", list:c + "list", onchange:buildRgxTree, onfocus:function(c) {
    return c.dom.setSelectionRange(0, c.dom.value.length);
  }, value:f || "", style:"min-width:72px;font-size:1em; height:2em"}, {name:c + "rgx", rgxRaw:cI(null), rgxTree:cI(null), history:cI([])}), datalist({id:c + "list"}, function(d) {
...
function mkRgxOrAnd() {
  return div({style:"color: white; margin:0 9px 0 0; display:flex; flex-wrap: wrap; align-items:center", title:"Replace 'or/and' with '||/&&' for easier mobile entry."}, input({id:"rgxOrAnd", type:"checkbox", checked:cF(function(c) {
    ...
    return c.value = !c.value;
  }, onchange:rebuildRgxTrees, title:"Replace 'or/and' with '||/&&' for easier mobile entry."}, {name:"rgxOrAnd", value:cI(!0)}), label({for:"rgxOrAnd", title:"Replace 'or/and' with '||/&&' for easier mobile entry."}, "allow or/and"));
}
...
function appBanner() {
  return div(header(div({class:"about", onclick:function(c) {
    return c.onOff = !c.onOff;
  }, title:"Usage hints, and credit where due.", content:cF(function(c) {
    return c.md.onOff ? "hide" : "Pro tips";
  })}, {name:"appHelpToggle", onOff:cI(!1)}), div({class:"headermain"}, span({class:"askhn"}, "Ask HN:"), span({class:"who"}, "Who Is Hiring?"))), helpList(appHelpEntry, "appHelpToggle"));
}
var appHelpEntry = "Click any job header to show or hide the full listing.{Once visible, double-click the job description to open the listing on HN in a new tab.{All filters are ANDed except as you direct within RegExp fields.{Your annotations are kept in local storage, so stick to one browser.{Works off page scrapes taken every couple of hours. E-mail <a href='mailto:kentilton@gmail.com'>Kenny</a> if they seem stopped.{HIRERS! Not seeing your listing? Scraper determination is fuzzy. E-mail <a href='mailto:kentilton@gmail.com'>Kenny</a> and we will sort it out.{RFEs welcome and can be raised <a href='https://github.com/kennytilton/whoshiring/issues'>here</a>. {This page is not affiliated with Hacker News, but thanks to the HN crew for their assistance. All screw-ups remain <a href='https://news.ycombinator.com/user?id=kennytilton'>kennytilton</a>'s.{Graphic design by <a href='https://www.mloboscoart.com'>Michael Lobosco</a>.{Built with <a href='https://github.com/kennytilton/matrix/blob/master/js/matrix/readme.md'>Matrix Inside</a>&trade;.".split("{");
